const getDataUrl = (): string => {
  ...
  return new URL(`${normalizedBase}data/ongoing-studies.json`, window.location.origin).toString();
};
...
async function loadOngoingStudies() {
  try {
    ...
    const studies = (data.studies as OngoingStudy[]) || [];
    ...
    studies.forEach((study) => {
      ...
      if (cardsContainer) {
        ...
        const tagsHtml = (study.tags || [])
          .map((tag: string) => `<span class="study-tag">${tag}</span>`)
          .join('');
...
function buildLinksCell(links?: StudyLinks): string {
  if (!links) return '';
...
function buildLinksList(links?: StudyLinks): string {
  if (!links) return '';

  const items: string[] = [];

  if (links.protocol) {
    items.push(`<li><strong>Protocol:</strong> <a href="${links.protocol}" target="_blank" rel="noopener">${links.protocol}</a></li>`);
  }
  if (links.registry) {
    items.push(`<li><strong>Registry:</strong> <a href="${links.registry}" target="_blank" rel="noopener">${links.registry}</a></li>`);
  }
  if (links.study_page) {
    items.push(`<li><strong>Study page:</strong> <a href="${links.study_page}" target="_blank" rel="noopener">${links.study_page}</a></li>`);
  }
  if (links.lay_summary) {
    items.push(`<li><strong>Lay summary:</strong> <a href="${links.lay_summary}" target="_blank" rel="noopener">${links.lay_summary}</a></li>`);
  }
